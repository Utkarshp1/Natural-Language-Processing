...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs = np.dot(np.expand_dims(error, axis=1), np.expand_dims(centerWordVec.T, axis=0))
...
def negSamplingLossAndGradient(
    ...
    U_required = outsideVectors[indices]
    U_required[1:] = -U_required[1:]
    temp = sigmoid(np.dot(U_required, centerWordVec))
    loss = -np.log(temp[0]) - np.sum(np.log(temp[1:]))

    gradCenterVec = -(1-temp[0])*outsideVectors[outsideWordIdx] + np.sum((1-temp[1:])*(-U_required[1:].T), axis=1)

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    unique_idx, idx_idx, idx_counts = np.unique(indices, return_counts=True, return_index=True)
    gradOutsideVecs[unique_idx] = np.dot(np.expand_dims((1-temp[idx_idx])*idx_counts, axis=1), np.expand_dims(centerWordVec.T, axis=0))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside_word in outsideWords:
        word_loss, word_gradCenterVecs, word_gradOutsideVectors = word2vecLossAndGradient(
            centerWordVec, word2Ind[outside_word], outsideVectors, dataset)
        loss += word_loss
        gradCenterVecs[currentWordIdx] += word_gradCenterVecs
        gradOutsideVectors += word_gradOutsideVectors
    ### END YOUR CODE
