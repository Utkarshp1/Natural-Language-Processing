...
\begin{shaded}
\begin{answer}
Since, $\bm y_{w}$ is a one-hot vector with 1 at the index of word $w$ and 0 otherwise, hence if we take element-wise product between $\bm y_{w}$ and $\hat{\bm y}$, we would get the predicted output probability of word $o$. Therefore, the two equations are equivalent.
\end{answer}
\end{shaded}
...
\end{equation*}
\begin{equation*}
    \frac{\partial J}{\partial \bm v_{c}} = \frac{\partial J}{\partial P} \frac{\partial P}{\partial \bm v_{c}}
\end{equation*}
From \ref{word2vec_condprob} we have, 
\begin{equation*}
    P(O=o \mid C=c) = \frac{\exp(\bm u_{o}^\top \bm v_c)}{\sum_{w \in \text{Vocab}} \exp(\bm u_{w}^\top \bm v_c)}
\end{equation*}
\begin{align*}
    \frac{\partial P}{\partial \bm v_{c}} &= P(O=o \mid C=c)\bm u_{o} - P(O=o \mid C=c)\bm u_{o} \frac{\sum_{x \in \text{Vocab}} \exp(\bm u_{x}^{T} \bm v_{c}) \bm u_{x}}{\sum_{w} \exp(\bm u_{w}^{T} \bm v_{c})} \\
    &= P(O=o \mid C=c)\left [ \bm u_{o} - \sum_{x \in \text{vocab}} \frac{\exp(\bm u_{x}^{T} \bm v_{c})}{\sum_{w} \exp(\bm u_{w}^{T} \bm v_{c})} \bm u_{x} \right] \\
    &= P(O = o | C = c) \left [ \bm u_{o} - \sum_{x \in \text{vocab}} P(O = x | C = c) \bm u_{x} \right] \\
    &= \hat{\bm y}^{T} y \left [\bm U \bm y - \bm U \bm \hat{\bm y}^{T} \right ]
\end{align*}
\begin{align*}
    \frac{\partial J}{\partial P} = - \frac{1}{P(O=o|C=c)} = -\frac{1}{\hat{\bm y}^{T} y}
\end{align*}
Therefore,
\begin{align*}
    \frac{\partial J}{\partial \bm v_{c}} &= -\frac{1}{\hat{\bm y}^{T} y} \hat{\bm y}^{T} y \left [\bm U \bm y - \bm U \bm \hat{\bm y} \right ] \\
    & = \left [\bm U \bm \hat{\bm y} - \bm U \bm y \right ] \\
    &= \bm U \left [\bm \hat{\bm y} -  \bm y \right]
\end{align*}
The gradient $\frac{\partial J}{\partial \bm v_{c}}$ will be zero when $\bm \hat{\bm y}^{T} -  \bm y$ will lie in the null space of $\bm U$. \\
The gradient contains the term $\bm \hat{\bm y}^{T} -  \bm y$ which represents how far the predicted probability for the outside word is from the true probability. When this difference approaches $0$, the word vectors $\bm v_{c}$ will stop updating. Until then it helps to update the vectors in the direction such that the true probability becomes equal to the predicted probability. 
\end{answer}
\end{shaded}
...
\textbf{Case 1:} When $w \neq o$: \\
\begin{align*}
\frac{\partial P}{\partial \bm u_{w}} &= \frac{exp(\bm u^T_{o} \bm v_{c}) exp(\bm u^T_{w} \bm v_{c})}{\left( \sum_{w} exp(\bm u^T_{w} \bm v_{c}) \right )^2}  \bm v_{c} \\
&= P(O = o | C = c) P(O = w | C = c) \bm v_{c} \\
\frac{\partial J}{\partial \bm u_{w}} &= \hat{\bm y}_{w} \bm v_{c}
\end{align*}
\textbf{Case 2:} When $w = o$: This case is similar to the previous part's derivation, so skipping some details \\
\begin{align*}
    \frac{\partial P}{\partial \bm u_{o}} &= P(O = o | C = c)\left [ 1 - P(O = o | C = c) \right] \bm v_{c} \\
    \frac{\partial J}{\partial \bm u_{w}} = \left [ \hat{\bm y}_{o} - \bm y_{o} \right ] \bm v_{c}
\end{align*}
Therefore,
\begin{align*}
    \frac{\partial J}{\partial \bm u_{w}} = \left [ \hat{\bm y}_{w} - \bm y_{w} \right ] \bm v_{c}
\end{align*}
where,
\begin{align*}
    \bm y_{w} = 
...
\begin{answer}
\begin{align*}
    \frac{\partial J}{\partial \bm U} = \left [\frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_1} \frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_2} \cdots \frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_{|\text{Vocab}|}} \right] = \bm v_{c} \left[\hat{\bm y} - \bm y \right]^T 
\end{align*}
\end{answer}
\end{shaded}
...
\begin{answer}
\begin{align*}
    \sigma ' (x) = \frac{1}{(1+e^{-x})^2}e^{-x} = \sigma (x) \left[ 1- \sigma (x) \right]
\end{align*}
\end{answer}
\end{shaded}
...
\begin{answer}
\begin{align*}
    \frac{\partial J}{\partial \bm v_{c}} &= \frac{-1}{\sigma (\bm u_{o}^T \bm v_{c})} \frac{\partial \sigma (\bm u_{o}^T \bm v_{c})}{\partial \bm u_{o}^T \bm v_{c}} \bm u_{o} + \sum_{s=1}^{K} \frac{1}{\sigma (\bm u_{w_{s}}^T \bm v_{c})} \frac{\partial \sigma (-\bm u_{w_{s}}^T \bm v_{c})}{\partial -\bm u_{w_{s}}^T \bm v_{c}} \bm u_{w_{s}} \\
    &= - \left[1- \sigma(\bm u_{o}^T \bm v_{c})  \right] \bm u_{o} + \sum_{s=1}^{K} \left[1- \sigma(-\bm u_{w_{s}}^T \bm v_{c})  \right] u_{w_{s}}
 \end{align*}
\begin{align*}
    \frac{\partial J}{\partial \bm u_{o}} = -\left [ 1- \sigma(\bm u_{o}^T \bm v_{c}) \right] \bm v_{c}
\end{align*}
\begin{align*}
    \frac{\partial J}{\partial \bm u_{w_{s}}} = \left[1- \sigma(-\bm u_{w_{s}}^T \bm v_{c})  \right] \bm v_{c}
\end{align*}
The quantity $\left[\bm 1 - \sigma \left ( \bm U_{o, \{w_{1}, \cdots w_{K}\}}^{T} \bm v_{c} \right) \right ]$, can be reused amongst all the above partial derivative computations. \\
This loss function is more efficient to compute than the naive-softmax loss because the denominator in the naive-softmax requires sum over all the words in the vocabulary. However, this loss requires to compute sum of just $K$ of the negative words from the vocabulary.
\end{answer}
\end{shaded}
...
\begin{answer}
\begin{align*}
    \frac{\partial J}{\partial \bm u_{w_{s}}} = \sum \limits_{\substack{i=0 \\ w_{i} = w_{s}}}^K \left[1- \sigma(-\bm u_{w_{s}}^T \bm v_{c})  \right] \bm v_{c}
\end{align*}
\end{answer}
\end{shaded}
...
\end{align*}
\begin{align*}
    {\frac{\partial \bm J_{\textrm{skip-gram}}(\bm v_c, w_{t-m},\ldots w_{t+m}, \bm U)} {\partial \bm v_w}} = \bm 0 \hspace{10pt} \text{when } w \neq c
\end{align*}
\end{answer}
\end{shaded}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{word_vectors.png}
...
\begin{answer}
\begin{itemize}
    \item The similar words such pairs such as (rain, snow), (amazing, wonderful), (sweet, tea, coffee) are clustered together.
    \item Words such as male, man, female, woman, queen, king are all over the place. They should have been clustered together.
    \item Adjectives such as enjoyable, annoying, sweet, amazing, wonderful, boring, great are not clustered together.
\end{itemize}
\end{answer}
\end{shaded}
